...
def plot_rel_spikes(m_spikes):
    rel_spikes = []

    for i_rbf in range(n_rbf):
        rel_spikes_rbf = (spike_mon.t[spike_mon.i == i_rbf]/ms) % (runtime/ms)
        plot(rel_spikes_rbf, range())
    ylabel('neurons_rbf Neuron ID')
    xlabel('Time (in ms)')
    yticks(range(n_rbf))
    ylim([-0.5, 1.5])
...
syn_stdp = Synapses(neurons_in, neurons_rbf, model='''w : 1
                                ti : 1
                                tj : 1
                                delta_t : 1
                                delta_t2 : 1
             ...
             on_pre='''x += w
                        ...
                        w_delta = ((1-b) * exp(-((delta_t-c)**2)/beta**2) + b)
                      ''',
             on_post='''
                          tj = t/(1*ms)
                          delta_t = ti - tj - d_back
                          ...
                          w_delta = ((1-b) * exp(-((delta_t-c)**2)/beta**2) + b)
                          w_new = w + lr * ((1-b) * exp(-((delta_t-c)**2)/beta**2) + b)''',
